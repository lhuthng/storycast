...
pub(crate) async fn key_text(app: &mut App, prompt: TextPrompt, key: KeyEvent, http: &reqwest::Client, job_tx: &tokio::sync::mpsc::UnboundedSender<Job>) -> bool {
        let mut p = prompt;
                ...
                if p.kind == TextKind::Command {
                    ...
                    return match command_key(&buf) {
                        ...
                        Some(Command::Key(code)) => {
                            Box::pin(super::handle_key(app, KeyEvent::new(code, KeyModifiers::empty()), http, job_tx)).await
                        }
                        Some(cmd) => {
