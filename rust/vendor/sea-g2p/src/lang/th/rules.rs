...
fn is_long(v: &str) -> bool {
    matches!(
        v,
        "aː" | "iː" | "ɯː" | "uː" | "eː" | "ɛː" | "oː" | "ɔː" | "ɤː" | "ia" | "ɯa" | "ua"
    )
}
...
fn class_of(c: char) -> Class {
    if CLASS_MID.contains(c) {
        Class::Mid
    } else if CLASS_HIGH.contains(c) {
        Class::High
    } else {
        Class::Low
    }
...
static FRAMES: Lazy<Vec<Frame>> = Lazy::new(|| {
    frames![
...
fn frame_readings(chars: &[char], i: usize, forced: Option<Class>) -> Vec<Reading> {
    ...
    for f in FRAMES.iter() {
        ...
        let c1 = chars[k];
        let onset_base = match INIT.get(&c1) {
            Some(o) => *o,
            None => continue,
        };
        k += 1;
        ...
        k = match starts_with_at(chars, k, f.ab) {
            Some(k) => k,
            None => continue,
        };
        ...
        k = match starts_with_at(chars, k, f.post) {
            Some(k) => k,
            None => continue,
        };
            ...
            match f.vowel {
                ...
                _ => {
                    ...
                    if k < chars.len() && matches!(chars[k], 'ย' | 'ว') {
                        ...
                        let taken =
                            !matches!(nxt, Some(n) if is_dependent(n) || PREPOSED.contains(n));
                        if taken {
                            coda = if chars[k] == 'ย' {
                                "j".into()
                            } else {
...
fn silent_lead_readings(chars: &[char], i: usize) -> Vec<Reading> {
    let mut out = Vec::new();
    let pre_len = if i < chars.len() && PREPOSED.contains(chars[i]) {
        1
    } else {
        0
    };
...
fn leading_readings(chars: &[char], i: usize) -> Vec<Reading> {
    ...
    if !INIT.contains_key(&c1) || class_of(c1) == Class::Low {
        return out;
    }
    ...
    if !SONORANT.contains(c2) {
        return out;
    }
    let pair: String = [c1, c2].iter().collect();
    if TRUE_CLUSTERS.contains(&pair.as_str()) || PSEUDO.contains_key(pair.as_str()) {
        return out;
    }
    if i + 2 >= chars.len() {
        return out;
