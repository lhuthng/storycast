...
static RE_ISO_FIX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(\d{2})T(\d{2})|(\d{2})Z\b").unwrap());

static RE_DAY_MONTH: Lazy<Regex> = Lazy::new(|| {
    Regex::new(&format!(
        r"(?<![a-zA-Z\d])(?<![a-zA-Z\d][.,])(\d{{1,2}}){}{{{}}}(\d{{1,2}})(?!\d|[.,]\d)",
        SHORT_DATE_SEP, 1
    ))
    .unwrap()
});
...
static RE_MONTH_YEAR: Lazy<Regex> = Lazy::new(|| {
    Regex::new(&format!(
        r"(?<![a-zA-Z\d])(?<![a-zA-Z\d][.,])(\d{{1,2}}){}{{{}}}(\d{{4}})(?!\d|[.,]\d)",
        DATE_SEP, 1
    ))
    .unwrap()
});
...
static RE_PERIOD_YEAR: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b([a-zA-Z]\d*)/(\d{4})\b").unwrap());
...
static RE_QUY_YEAR: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(quý)\s+(\d{1,2}|IV|III|II|I)\s*/\s*(\d{4})\b").unwrap());
...
static RE_HOUR_CONTEXT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(\d+)g\s*(sáng|trưa|chiều|tối|khuya)\b").unwrap());

static RE_LUC_HOUR: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\blúc\s*(\d+)g\b").unwrap());
...
pub static NUM_WORDS: Lazy<std::collections::HashSet<&'static str>> = Lazy::new(|| {
    [
        "không", "một", "hai", "ba", "bốn", "năm", "sáu", "bảy", "tám", "chín", "mười", "mươi",
        "mốt", "lăm", "tư", "trăm", "nghìn", "ngàn", "triệu", "tỷ",
    ]
    .into_iter()
    .collect()
});
...
static RE_REDUNDANT_NGAY: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bngày\s+ngày\s+(\S+)").unwrap());

static RE_REDUNDANT_THANG: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\btháng\s+tháng\s+(\S+)").unwrap());

static RE_REDUNDANT_NAM: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bnăm\s+năm\s+(\S+)").unwrap());
...
static RE_REDUNDANT_HOM_NGAY: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bhôm\s+ngày\s+(\S+)").unwrap());

static RE_REDUNDANT_MUNG_NGAY: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b(mùng|mồng)\s+ngày\s+(\S+)").unwrap());
...
fn get_context_words(text: &str, start: usize, end: usize, window_size: usize) -> Vec<String> {
    ...
    let left_start = if left_part.len() > window_size {
        left_part.len() - window_size
    } else {
        0
    };
...
pub fn normalize_date(text: &str) -> String {
    ...
    result = RE_YYYY_MM_DD
        .replace_all(&result, |caps: &Captures| {
            ...
            if is_valid_date(day, month) {
                let m_val = if month.parse::<i32>().unwrap_or(0) == 4 {
                    "tư".to_string()
                } else {
                    n2w(&month.parse::<i32>().unwrap_or(0).to_string())
                };
                format!(
                    "ngày {} tháng {} năm {}",
                    n2w(&day.parse::<i32>().unwrap_or(0).to_string()),
                    m_val,
                    n2w(year)
                )
            } else {
            ...
            let prefix = if code_lower.starts_with('q') && code.len() <= 2 {
                ...
                for c in code.chars() {
                    ...
                    if c.is_ascii_digit() {
                        ...
                        crate::lang::vi::resources::VI_LETTER_NAMES.get(cl.as_str())
                    {
    ...
    result = RE_DAY_MONTH
        .replace_all(&current_text, |caps: &Captures| {
            ...
            let context_words =
                get_context_words(&current_text, full_match.start(), full_match.end(), 3);
            let math_symbols = [
                "+", "-", "*", "x", "×", "/", "=", ">", "<", "≥", "≤", "≈", "±",
            ];
            ...
            if is_valid
                && context_words
                    .iter()
                    .any(|w: &String| DATE_KEYWORDS.contains(w.as_str()))
            {
                let m_val = if b == 4 {
                    "tư".to_string()
                } else {
            ...
            if context_words
                .iter()
                .any(|w: &String| MATH_KEYWORDS.contains(w.as_str()))
                || context_words
                    .iter()
                    .any(|w: &String| math_symbols.contains(&w.as_str()))
            {
    ...
    result = RE_TIME
        .replace_all(&result, |caps: &Captures| {
            ...
            if m_int >= 0 && m_int < 60 {
                ...
                let h_words = if is_min_sec {
                    n2w(h_str)
                } else {
                    n2w(norm_time_part(h_str))
                };
