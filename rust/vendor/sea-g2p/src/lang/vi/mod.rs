...
use fancy_regex::{Captures as FCaps, Regex as FRegex};
...
use crate::lang::vi::misc::{
    expand_standalone_letters, expand_weekday_abbr, normalize_others, RE_ACRONYM,
    RE_ACRONYMS_EXCEPTIONS,
};
use crate::lang::vi::numerical::{expand_multiply_number, normalize_number_vi, RE_MULTIPLY};
use crate::lang::vi::resources::{COMBINED_EXCEPTIONS, MEASUREMENT_KEY_VI};
use crate::lang::vi::technical::{normalize_emails, normalize_technical, RE_EMAIL, RE_TECHNICAL};
use crate::lang::vi::units::{
    expand_compound_units, expand_height_weight, expand_power_of_ten, expand_scientific_notation,
    expand_units_and_currency, fix_english_style_numbers,
};
...
static RE_MISSING_SPACE_AFTER_PUNCT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"([.,!?;:])([^\s\d<])").unwrap());
static RE_INTERNAL_EN_TAG: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)(?s)(__start_en__.*?__end_en__|<en>.*?</en>)").unwrap());
...
// read "âm".
static RE_MATH_BIN_MINUS: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<=[0-9A-Za-z)])\s*[-–—]\s*(?=[0-9A-Za-z(∫√])").unwrap());
...
static MATH_FUNCS: Lazy<std::collections::HashSet<&'static str>> = Lazy::new(|| {
    [
        ...
        "arccos", "arctan", "asin", "acos", "atan", "log", "ln", "lg", "exp", "lim", "max", "min",
        "sup", "inf", "det", "dim", "deg", "gcd", "lcm", "mod", "arg", "sgn", "rank", "tr",
    ]
    .into_iter()
    .collect()
});
...
// read ("đê ích").
static MATH_DIFFS: Lazy<std::collections::HashSet<&'static str>> =
    Lazy::new(|| ["dx", "dy", "dz", "du", "dv", "dt"].into_iter().collect());
...
fn classify_math_token(tok: &str) -> (bool, bool) {
    ...
    let pure_alpha_multi =
        runs.len() == 1 && runs[0].chars().count() >= 2 && !has_math_evidence && {
        ...
        .map(|line| {
            ...
            let mathy: Vec<bool> = (0..toks.len())
                .map(|i| {
                    ...
                    let next_op =
                        i + 1 < toks.len() && classes[i + 1].0 && has_operator_char(toks[i + 1]);
                    prev_op || next_op
                })
                .collect();
            ...
            while i < toks.len() {
                ...
                if (j - i >= 2 && strong) || (j - i == 1 && strong_non_eq) {
                    ...
                    for (k, t) in toks[i..j].iter().enumerate() {
                        ...
                        let prev_numeric =
                            k == 0 || toks[i + k - 1].chars().any(|c| c.is_ascii_digit());
                        parts.push(split_math_token(t, prev_numeric));
                    }
                    let s = parts.join(" ");
                    let s = RE_MATH_FACTORIAL
                        .replace_all(&s, " giai thừa ")
                        .into_owned();
...
static RE_CONTEXT_TRU: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(b\u1eb1ng|t\u00ednh|k\u1ebft qu\u1ea3)\s+(\d+(?:[.,]\d+)?)\s*[-\u2013\u2014]\s*(\d+(?:[.,]\d+)?)\b").unwrap()
});
static RE_CONTEXT_TRU_POST: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(\d+(?:[.,]\d+)?)\s*[-\u2013\u2014]\s*(\d+(?:[.,]\d+)?)\s+(b\u1eb1ng|t\u00ednh|k\u1ebft qu\u1ea3)\b").unwrap()
});
static RE_CONTEXT_DEN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(t\u1eeb|kho\u1ea3ng|trong)\s+(\d+(?:[.,]\d+)?)\s*[-\u2013\u2014]\s*(\d+(?:[.,]\d+)?)\b").unwrap()
});
static RE_EQ_MINUS: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"([\d./]+)\s*[-\u2013\u2014]\s*([\d./]+)\s*=").unwrap());
static RE_EQ_NEG: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"=\s*[-\u2013\u2014](\d+(?:[./]\d+)?)").unwrap());
static RE_PHONE_WITH_DASH: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b(0\d{2,3})[\u2013\-\u2014](\d{3,4})[\u2013\-\u2014](\d{4})\b").unwrap()
});
static RE_POWER_OF_TEN_IMPLICIT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b10\^([-+]?\d+)\b").unwrap());
...
static RE_NUMERIC_DASH_GROUPS: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b\d+(?:[\u2013\-\u2014]\d+){2,}\b").unwrap());
static RE_PHONE_SPACE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b0\d{2,3}(?:\s\d{3}){2}\b").unwrap());
...
static RE_RANGE_PCT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\d+(?:[.,]\d+)?)\s*[-–—]\s*(\d+(?:[.,]\d+)?)\s*%").unwrap());
...
static RE_PCT_NEG: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<![\d.,])[-–—]\s?(\d+(?:[.,]\d+)?)\s*%").unwrap());
...
static RE_SCORE_KW: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(thắng|thua|hòa|hoà|tỉ số|tỷ số|chung cuộc|đánh bại|cầm hòa|cầm hoà)\s+(\d{1,2})\s*[-–—]\s*(\d{1,2})\b").unwrap()
});
static RE_SCORE_TEAMS: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(\p{Lu}\p{L}+(?:\s\p{Lu}\p{L}+)?)\s(\d{1,2})\s*[-–—]\s*(\d{1,2})\s(\p{Lu}\p{L}+)")
        .unwrap()
});
...
const SCORE_EXCLUDE: [&str; 26] = [
    "điều", "khoản", "chương", "mục", "phần", "điểm", "tiết", "tập", "quyển", "hồi", "kỳ", "kì",
    "quý", "tháng", "ngày", "năm", "tuần", "từ", "khoảng", "trong", "bài", "câu", "trang", "dòng",
    "khóa", "khoá",
];

// Service numbers 1800/1900 are read figure by figure, never as a cardinal.
static RE_HOTLINE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b(1800|1900)[\s.\-–—]?(\d{3,6})\b").unwrap());
...
static RE_LANDLINE: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<![\d.,])\(?(?:\+84\s?|0)\d{1,3}\)?\s\d{3,4}\s\d{3,4}(?!\d)").unwrap()
});
...
static RE_MONEY_TR: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"\b(\d{1,4})tr(\d)?(?![\w])").unwrap());
...
static RE_RANGE: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<!\d)(?<!\d[,.])(?<![a-zA-Z])(\d{1,15}(?:[,.]\d{1,15})?)(\s*)[\u2013\-\u2014](\s*)(\d{1,15}(?:[,.]\d{1,15})?)(?!\d)(?![.,]\d)(?![a-zA-Z])").unwrap()
});
static RE_DASH_TO_COMMA: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<=\s)[\u2013\-\u2014](?=\s)").unwrap());
...
static RE_MATH_MINUS_SUP: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(
        r"(?<=[\u00b2\u00b3\u2074\u2075\u2076\u2077\u2078\u2079\u207f\u2071])\s*[-\u2013\u2014]\s*",
    )
    .unwrap()
});
static RE_MATH_MINUS_COEF: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"\s[-\u2013\u2014]\s(?=\d+[a-zA-Z])").unwrap());
static RE_MATH_MINUS_COEFL: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<=\d[a-zA-Z])\s[-\u2013\u2014]\s(?=\d)").unwrap());
...
// "âm s" plus "in".
static RE_NEG_VAR1: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<=[(\[{=\u00b1+*/\u00d7\u00f7])[-\u2013\u2014]((?:sin|cos|tan|cot|log|ln|lim)\b|[a-zA-Z]\b)").unwrap()
});
static RE_NEG_VAR2: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<=[(\[{=\u00b1+*/\u00d7\u00f7]\s)[-\u2013\u2014]((?:sin|cos|tan|cot|log|ln|lim)\b|[a-zA-Z]\b)").unwrap()
});
// "-sin x" mid-sentence ("là -sin x"): a minus after whitespace and directly
// before a trigonometric function name reads as "âm sin".
static RE_NEG_FUNC: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(r"(?<=\s)[-\u2013\u2014](?=(?:sin|cos|tan|cot|log|ln|lim)\b)").unwrap()
});
...
static RE_FACTORIAL_INLINE: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<=\b[0-9a-zA-Z])!(?=\s*[)=/,(+\u00d7*-])").unwrap());
// A run of consecutive Unicode superscripts ("10²³") is ONE exponent
// ("mười mũ hai mươi ba"), not the sequence "bình phương lập phương".
static RE_SUPERSCRIPT_RUN: Lazy<Regex> = Lazy::new(|| {
...
static RE_SUPERSCRIPT_SIGNED: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"([\u207b\u207a])([\u2070\u00b9\u00b2\u00b3\u2074\u2075\u2076\u2077\u2078\u2079]+)")
        .unwrap()
});
...
static RE_MINUS_SINGLE_VARS: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b([a-z])\s+[-–—]\s+([a-z])\b").unwrap());
static RE_FLOAT_WITH_COMMA: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<![\d.])(\d+(?:\.\d{3})*),(\d+)(%)?").unwrap());
static RE_STRIP_DOT_SEP: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<![\d.])\d+(?:\.\d{3})+(?![\d.])").unwrap());
static RE_LONG_NUM: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<!\d)(?<!\d[,.])([-–—]?)(\d{7,})(?!\d)(?![.,]\d)").unwrap());
static RE_CAMEL_CASE: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<=[a-z])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])").unwrap());
...
fn cleanup_whitespace(text: &str) -> String {
    ...
    res = RE_MISSING_SPACE_AFTER_PUNCT
        .replace_all(&res, "$1 $2")
        .into_owned();
...
fn expand_money_slang(text: &str) -> String {
    RE_MONEY_TR
        .replace_all(text, |caps: &FCaps| {
            let x = crate::lang::vi::num2vi::n2w(caps.get(1).unwrap().as_str());
            match caps.get(2) {
                Some(y) => format!(
                    " {} triệu {} trăm nghìn ",
                    x,
                    crate::lang::vi::num2vi::n2w(y.as_str())
                ),
                None => format!(" {} triệu ", x),
            }
        })
...
fn split_concatenated_terms(text: &str) -> String {
    ...
    re_potential
        .replace_all(text, |caps: &Captures| {
            ...
            if protected || MEASUREMENT_KEY_VI.contains_key(word.to_lowercase().as_str()) {
                word.to_string()
            } else {
...
static RE_EARLY_EXCEPTIONS: Lazy<Option<Regex>> = Lazy::new(|| {
    let keys: Vec<String> = COMBINED_EXCEPTIONS
        .keys()
        .filter(|k: &&String| {
            k.contains('&')
...
fn stage_detect_context(text: &str, force_vi: bool) -> Ctx {
    let vi_ctx = text
        .chars()
        .any(|c: char| c.is_alphabetic() && !c.is_ascii());

    let lowercase_wordish = RE_WORDISH
        .find_iter(text)
        .filter(|m: &regex::Match| m.as_str().chars().any(|c: char| c.is_ascii_lowercase()))
        .take(3)
        .count();
    let dictionary_words = RE_WORDISH
        .find_iter(text)
    ...
    let en_ctx = !force_vi && !vi_ctx && (lowercase_wordish >= 3 || dictionary_words >= 2);
...
fn stage_superscripts(text: &str) -> String {
    ...
    out = RE_SUPERSCRIPT_SIGNED
        .replace_all(&out, |caps: &Captures| {
            let sign = if caps.get(1).unwrap().as_str() == "\u{207b}" {
                "trừ "
            } else {
                ""
            };
...
fn stage_math(text: &str, ctx: Ctx) -> String {
    ...
    out = RE_FACTORIAL_INLINE
        .replace_all(&out, " giai thừa ")
        .into_owned();
    ...
    RE_MINUS_SINGLE_VARS
        .replace_all(&out, "$1 trừ $2")
        .into_owned()
}
...
fn stage_arithmetic(text: &str) -> String {
    ...
    out = RE_MULTIPLY
        .replace_all(&out, |caps: &FCaps| {
            expand_multiply_number(caps.get(0).unwrap().as_str())
        })
        .to_string();

    out = RE_CONTEXT_TRU
        .replace_all(&out, " $1 $2 trừ $3 ")
        .into_owned();
    out = RE_CONTEXT_TRU_POST
        .replace_all(&out, " $1 trừ $2 $3 ")
        .into_owned();
    out = RE_CONTEXT_DEN
        .replace_all(&out, " $1 $2 đến $3 ")
        .into_owned();
...
fn stage_phones(text: &str) -> String {
    ...
    let mut out = RE_NUMERIC_DASH_GROUPS
        .replace_all(text, |caps: &Captures| {
            caps.get(0)
                .unwrap()
                .as_str()
...
fn stage_ranges_signs(text: &str) -> String {
    let mut out = RE_POWER_OF_TEN_IMPLICIT
        .replace_all(text, |caps: &Captures| {
            ...
            if let Some(rest) = exp.strip_prefix('-') {
                ...
                format!(
                    "mười mũ {}",
                    crate::lang::vi::num2vi::n2w(&exp.replace('+', ""))
                )
            }
        })
        .into_owned();
...
fn stage_units(text: &str, ctx: Ctx) -> String {
    ...
    out = RE_LONG_NUM
        .replace_all(&out, |caps: &FCaps| {
            let sign = if caps.get(1).unwrap().as_str().is_empty() {
                ""
            } else {
                "âm "
            };
...
fn stage_decimals(text: &str) -> String {
    ...
    out = RE_FLOAT_WITH_COMMA
        .replace_all(&out, |caps: &FCaps| {
            let int_part =
                crate::lang::vi::num2vi::n2w(&caps.get(1).unwrap().as_str().replace('.', ""));
            ...
            let mut res = if dec_part.is_empty() {
                ...
                format!(
                    "{} phẩy {}",
                    int_part,
                    crate::lang::vi::num2vi::n2w_decimal(dec_part)
                )
            };
...
pub fn clean_vietnamese_text_ctx(text: &str, force_vi: bool) -> String {
    ...
    current_text = RE_EMAIL
        .replace_all(&temp_email, |caps: &FCaps| {
            let orig = caps.get(0).unwrap().as_str();
    ...
    current_text = RE_TECHNICAL
        .replace_all(&temp_tech, |caps: &FCaps| {
            let orig = caps.get(0).unwrap().as_str();
            ...
            if orig.contains('-')
                && orig
                    .chars()
                    .all(|c: char| c.is_ascii_alphabetic() || c == '-')
    ...
    if let Some(re) = RE_EARLY_EXCEPTIONS.as_ref() {
        ...
        current_text = re
            .replace_all(&temp_exc, |caps: &Captures| {
                let orig = caps.get(0).unwrap().as_str();
                let val = COMBINED_EXCEPTIONS
                    .get(orig)
                    .cloned()
                    .unwrap_or(orig.to_string());
                protect(val, &mut mask_map)
    ...
    current_text = RE_INTERNAL_EN_TAG
        .replace_all(&temp_text3, |caps: &Captures| {
    ...
    if current_text.contains('.') {
        current_text = RE_DOT_BETWEEN_DIGITS
            .replace_all(&current_text, |caps: &Captures| {
                format!(
                    "{} chấm {}",
                    caps.get(1).unwrap().as_str(),
    ...
    current_text = current_text
        .replace("__start_en__", "<en>")
        .replace("__end_en__", "</en>");
...
impl Normalizer {
    ...
    pub fn normalize(&self, text: &str, punc_norm: bool) -> String {
        ...
        if had_math {
            current_text = RE_MATH_TAG
                .replace_all(&current_text, |caps: &Captures| {
                    let inner = split_math_letters(caps.get(1).unwrap().as_str());
                    let inner = RE_MATH_FACTORIAL
                        .replace_all(&inner, " giai thừa ")
                        .into_owned();
        ...
        current_text = RE_EN_TAG
            .replace_all(&temp_text, |caps: &Captures| {
    ...
    pub fn normalize_batch(
        ...
        py.allow_threads(|| {
            ...
            Ok(texts
                .into_par_iter()
                .map(|t| self.normalize(&t, punc_norm))
                .collect())
        })
    }
}
