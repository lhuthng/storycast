...
static RE_SANDWICH_24: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b([a-z]+)([24])([a-z]+)\b").unwrap());

pub fn expand_sandwich_digits(text: &str) -> String {
    if !text.chars().any(|c: char| c == '2' || c == '4') {
    ...
    RE_SANDWICH_24
        .replace_all(text, |caps: &Captures| {
            ...
            if matches!(
                left,
                "sin" | "cos" | "tan" | "cot" | "log" | "lg" | "ln" | "lim"
            ) {
                return caps.get(0).unwrap().as_str().to_string();
            }
            let d = if caps.get(2).unwrap().as_str() == "2" {
                "two"
            } else {
                "four"
            };
            ...
            format!(
                "{} {} {}",
                wrap(caps.get(1).unwrap().as_str()),
                d,
                wrap(caps.get(3).unwrap().as_str())
            )
        })
        .into_owned()
}
...
static RE_EN_TIME: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b(\d{1,2}):([0-5]\d)\b").unwrap());
static RE_EN_PERCENT: Lazy<Regex> = Lazy::new(|| Regex::new(r"(\d+(?:\.\d+)?)\s*%").unwrap());
static RE_EN_DOLLAR: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\$\s*(\d+(?:,\d{3})*(?:\.\d+)?)").unwrap());
static RE_EN_UNIT: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?i)\b(\d+(?:\.\d+)?)\s*(tb|gb|mb|kb|ghz|mhz|khz|hz|kg|km|cm|mm|ml|mah|dpi|fps|mph)\b",
    )
    .unwrap()
});
static RE_EN_SLASH_NUM: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b(\d{1,4})/(\d{1,4})\b").unwrap());
...
static RE_EN_ALNUM: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b[A-Za-z0-9]+\b").unwrap());
static RE_EN_SUB_TOKENS: Lazy<Regex> = Lazy::new(|| Regex::new(r"[A-Za-z]+|\d+").unwrap());
static RE_EN_NUMBER: Lazy<Regex> = Lazy::new(|| Regex::new(r"\d+(?:,\d{3})*(?:\.\d+)?").unwrap());
...
pub fn english_prenormalize(text: &str) -> String {
    ...
    t = RE_EN_SLASH_NUM
        .replace_all(&t, |caps: &Captures| {
            format!(
                "{} slash {}",
                n2w_en(caps.get(1).unwrap().as_str()),
                n2w_en(caps.get(2).unwrap().as_str())
            )
        })
        .into_owned();
    ...
    t = RE_EN_NUMBER
        .replace_all(&t, |caps: &Captures| n2w_en(caps.get(0).unwrap().as_str()))
        .into_owned();
    ...
    {
        static RE_EN_INNER_DOT: Lazy<Regex> =
