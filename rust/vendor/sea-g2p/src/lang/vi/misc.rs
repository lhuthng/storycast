...
use crate::lang::vi::resources::{
    ABBRS, COMBINED_EXCEPTIONS, CURRENCY_KEY, DOMAIN_SUFFIX_MAP, ENGLISH_AMPERSAND,
    MEASUREMENT_KEY_VI, ROMAN_KEYWORDS, ROMAN_NUMERALS, SUBSCRIPTS_MAP, SUPERSCRIPTS_MAP,
    SYMBOLS_MAP, VI_ABBREV, VI_LETTER_NAMES,
};
...
static RE_ROMAN_LIST_MARKER: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?m)^([ \t]*)([IVXLCDM]+)(\.[ \t]+)(?=(\p{Lu}+))").unwrap());
...
static RE_ROMAN_SINGLE: Lazy<FRegex> = Lazy::new(|| FRegex::new(r"\b[IVX]\b(?!['’])").unwrap());
...
static RE_TITLE_DOT: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"\b(TS|GS|BS|ThS|PGS|KS|ĐH)\.\s+(?=\p{Lu})").unwrap());
...
static RE_DISTRICT_DOT: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"\bQ\.\s*(?=\d|\p{Lu}\p{Ll})").unwrap());
static RE_WARD_DOT: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"\bP\.\s*(?=\d|\p{Lu}\p{Ll})").unwrap());
...
static RE_STANDALONE_LETTER: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<![\''])\b([a-zA-Z])(?!['’]\w)\b(\.?)").unwrap());
...
pub static RE_ACRONYM: Lazy<FRegex> = Lazy::new(|| {
    FRegex::new(&format!(
        r"\b(?=[A-Z{}a-z{}0-9]*[A-Z{}])(?:[A-Z{}][a-z{}]?\d*){{2,}}(?:(?<=\d)[a-z{}]+)?\b",
        VI_UPPER, VI_UPPER, VI_UPPER, VI_UPPER, "đăâêôơư", "đăâêôơư"
    ))
    .unwrap()
});
static RE_VERSION: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<![-\u2013\u2014])\b(\d+(?:\.\d+){2,})\b").unwrap());
...
// markdown table ("| cột |").
static RE_ABS: Lazy<FRegex> = Lazy::new(|| FRegex::new(r"\|(\S(?:[^|]*\S)?)\|").unwrap());
static RE_PRIME_DIGIT: Lazy<FRegex> =
    Lazy::new(|| FRegex::new(r"(?<=\d)(['\u2019]+|[\x22\u201D])").unwrap());

// ─ Simple patterns (regex crate — Thompson NFA, fast) ──────────────────
static RE_LETTER: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(?i)(chữ|chữ cái|kí tự|ký tự)\s+(['"]?)([a-z])(['"]?)\b"#).unwrap());
static RE_ALPHANUMERIC: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b(\d+)([a-zA-Z])\b").unwrap());
// English acronyms and brands joined by "&" (R&D, R & D, AT&T, S&P).
static RE_AMPERSAND_ACRONYM: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\b([a-z]{1,4})\s*&\s*([a-z]{1,4})\b").unwrap());
...
static RE_SIZE_LABEL: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?i)\b(size|cỡ)\s+((?:xxxl|xxl|xl|xs|[sml])(?:\s*/\s*(?:xxxl|xxl|xl|xs|[sml]))*)\b",
    )
    .unwrap()
});
...
static RE_LETTER_DIGIT: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b([a-zA-Z])(\d+)\b").unwrap());
static RE_BRACKETS: Lazy<Regex> = Lazy::new(|| Regex::new(r"[\(\[\{]\s*(.*?)\s*[\)\]\}]").unwrap());
static RE_STRIP_BRACKETS: Lazy<Regex> = Lazy::new(|| Regex::new(r"[\[\]\(\)\{\}]").unwrap());
static RE_TEMP_C_NEG: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)-(\d+(?:[.,]\d+)?)\s*°\s*c\b").unwrap());
static RE_TEMP_F_NEG: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)-(\d+(?:[.,]\d+)?)\s*°\s*f\b").unwrap());
static RE_TEMP_C: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)(\d+(?:[.,]\d+)?)\s*°\s*c\b").unwrap());
static RE_TEMP_F: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)(\d+(?:[.,]\d+)?)\s*°\s*f\b").unwrap());
...
static RE_CLEAN_OTHERS: Lazy<Regex> = Lazy::new(|| {
    ...
    Regex::new(&format!(
        r"[^a-zA-Z0-9\s{}.,!?_\'\'-]",
        crate::lang::vi::resources::VI_LETTERS
    ))
    .unwrap()
});
static RE_CLEAN_QUOTES: Lazy<Regex> = Lazy::new(|| Regex::new(r#"[“”„]"#).unwrap());
static RE_CLEAN_QUOTES_EDGES: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(^|[\s.,!?;:])[\u2018\u2019']+|[\u2018\u2019']+($|[\s.,!?;:])").unwrap()
});
static RE_COLON_SEMICOLON: Lazy<Regex> = Lazy::new(|| Regex::new(r"[:;]").unwrap());
static RE_UNIT_POWERS: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b([a-zA-Z]+)\^([-+]?\d+)\b").unwrap());
pub static RE_ACRONYMS_EXCEPTIONS: Lazy<Regex> = Lazy::new(|| {
    let mut keys: Vec<String> = COMBINED_EXCEPTIONS
        .keys()
    ...
    let pattern = keys
        .iter()
        .map(|k: &String| format!(r"\b{}\b", regex::escape(k)))
        .collect::<Vec<String>>()
        .join("|");
    Regex::new(&pattern).unwrap()
});
pub static DOMAIN_SUFFIXES_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\.(com|vn|net|org|edu|gov|io|biz|info)\b").unwrap());
...
// See issue #177.
static RE_ACRONYMS_SPLIT: Lazy<regex::Regex> =
...
static RE_CODE_DIGITS: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b([A-Z]{2,6})-(\d{3,6})\b").unwrap());
// "#45021" (order or ticket number): drop the "#" rather than saying "thăng",
// and read the digits individually.
static RE_HASH_ID: Lazy<Regex> = Lazy::new(|| Regex::new(r"#(\d{3,8})\b").unwrap());
...
pub fn expand_unit_powers(text: &str) -> String {
    RE_UNIT_POWERS
        .replace_all(text, |caps: &Captures| {
            ...
            let full_base = MEASUREMENT_KEY_VI
                .get(base_lower.as_str())
...
static RE_WEEKDAY_CHAIN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(thứ (?:hai|ba|tư|năm|sáu|bảy))(\s*(?:,|và|-|–|đến|tới)\s*)(?:T([2-7])|CN)\b")
        .unwrap()
});
...
pub fn expand_weekday_abbr(text: &str) -> String {
    if !text.contains('T') && !text.contains("CN") {
        return text.to_string();
    }
    let mut res = RE_WEEKDAY_LEAD
        .replace_all(text, |caps: &Captures| {
...
pub fn normalize_acronyms(text: &str, en_ctx: bool) -> String {
    ...
    for i in (0..final_parts.len()).step_by(2) {
        let s = final_parts[i];
        let sep = if i + 1 < final_parts.len() {
            final_parts[i + 1]
        } else {
            ""
        };
        ...
        //     ("CO2" -> "xê ô hai").
        let letter_tokens: Vec<&&str> = words
            .iter()
        ...
        if !is_all_caps {
            processed_s = RE_ACRONYM
                .replace_all(&processed_s, |caps: &FCaps| {
                    let word = caps.get(0).unwrap().as_str();
                    if word.chars().all(|c: char| c.is_ascii_digit()) {
                    ...
                    let has_vi_letter = word
                        .chars()
                        .any(|c: char| !c.is_ascii() && c.is_alphabetic());
                    let is_mixed_case = word.chars().any(|c: char| c.is_lowercase())
                        && word.chars().any(|c: char| c.is_uppercase());
                    ...
                    if has_vi_letter || is_mixed_case || has_subscript {
                        ...
                        for c in word.chars() {
                            let cl = c.to_lowercase().to_string();
                            if c.is_ascii_digit() {
                                parts.push(n2w_single(&c.to_string()));
                            } else if let Some(name) = VI_LETTER_NAMES.get(cl.as_str()) {
                                parts.push(name.to_string());
                            } else if let Some(sub_name) = SUBSCRIPTS_MAP.get(&c) {
                                parts.push(sub_name.trim().to_string());
                    ...
                    if word
                        .chars()
                        .any(|c: char| c.is_ascii_digit() || (c >= '₀' && c <= '₉'))
                    {
                        let res: Vec<String> = word
                            .chars()
                            .map(|c: char| {
                                if c.is_ascii_digit() {
                                    n2w_single(&c.to_string())
    ...
    RE_ALPHANUMERIC
        .replace_all(text, |caps: &Captures| {
            ...
            if let Some(name) = VI_LETTER_NAMES.get(char.as_str()) {
                ...
                if char == "d"
                    && (text.to_lowercase().contains("quốc lộ")
    ...
    RE_AMPERSAND_ACRONYM
        .replace_all(text, |caps: &Captures| {
            ...
            if ENGLISH_AMPERSAND.contains(key.as_str()) {
                ...
                let marked = crate::lang::vi::technical::en_marked(&format!(
                    "{} and {}",
                    spell(l),
                    spell(r)
                ));
...
pub fn normalize_others(text: &str, en_ctx: bool) -> String {
    ...
    let text = RE_ABS
        .replace_all(&text, " giá trị tuyệt đối của $1 ")
        .into_owned();
        ...
        RE_ACRONYMS_EXCEPTIONS
            .replace_all(&text, |caps: &Captures| {
                COMBINED_EXCEPTIONS
                    ...
                    .cloned()
                    .unwrap_or(caps.get(0).unwrap().as_str().to_string())
            })
            .into_owned()
    };

    res = normalize_slashes(&res);
    res = DOMAIN_SUFFIXES_RE
        .replace_all(&res, |caps: &Captures| {
            let suffix = DOMAIN_SUFFIX_MAP
                .get(caps.get(1).unwrap().as_str().to_lowercase().as_str())
                .copied()
                .unwrap_or("");
            format!(
                " chấm {} ",
    ...
    res = RE_ROMAN_LIST_MARKER
        .replace_all(&res, |caps: &FCaps| {
            ...
            if value <= 0 || value > ROMAN_MARKER_MAX || (single && head_upper.chars().count() < 2)
            {
    ...
    res = RE_ROMAN_NUMBER
        .replace_all(&roman_src, |caps: &FCaps| {
    ...
    res = RE_ROMAN_SINGLE
        .replace_all(&roman1_src, |caps: &FCaps| {
            let m = caps.get(0).unwrap();
