...
use crate::lang::vi::num2vi::{n2w, n2w_decimal, n2w_single};
...
pub static RE_MULTIPLY: Lazy<FRegex> = Lazy::new(|| {
    ...
    FRegex::new(r"(?i)\d+(?:\s*[a-zA-Zμµ²³°]+\d*)?(?:\s*[x×]\s*\d+(?:\s*[a-zA-Zμµ²³°]+\d*)?)+")
        .unwrap()
});
...
static RE_ORDINAL: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)(thứ|hạng)(\s+)(\d+)\b").unwrap());

static RE_PHONE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"((\+84|84|0|0084)(3|5|7|8|9)[0-9]{8})").unwrap());

static RE_DOT_SEP: Lazy<Regex> = Lazy::new(|| Regex::new(r"\d+(\.\d{3})+").unwrap());
...
pub fn num_to_words(number: &str, negative: bool) -> String {
        ...
        && number.len() > 1
    {
        ...
        return format!("{}{}", neg_prefix, n2w_single(number))
            .trim()
            .to_string();
    }
...
pub fn normalize_number_vi(text: &str) -> String {
    ...
    result = RE_NUMBER
        .replace_all(&temp_result, |caps: &FCaps| {
            let full_match = caps.get(0).unwrap();
            let start = full_match.start();
            let prefix_char = if start > 0 {
                temp_result
                    .as_bytes()
                    .get(start - 1)
                    .map(|&b: &u8| b as char)
            ...
            if let Some(_neg) = neg_symbol {
                if prefix_char.is_none()
                    || prefix_char.unwrap().is_whitespace()
                    || "([;,. ".contains(prefix_char.unwrap())
                {
                    is_neg = true;
                }
            }
