...
| A box is **online and healthy** but takes no work, for ever | check the `state` column for **relaxed**, somebody pressed `z` on it, or it was parked in an earlier session and the flag is in `machines.json`, so it survives restarts. `z` on the selected box puts it back to work. This is deliberately not `offline`: the box is answering and simply being withheld from, which is why the word in the pane matters |
| A parked box still shows `offline` or `error` instead of **relaxed** | correct. A fault outranks the park, the box broke while it was parked, and that is the fact worth acting on |
...
| A freshly launched box shows **awaiting-ip** | correct, not a fault, and it is the first thing every launch shows. `RunInstances` returns before EC2 assigns a public address; until it does the box is keyed by its instance id, the address column reads `none`, and nothing dials it. The address arrives within a few seconds, and the inductor is reading the account every 15 s while it waits: the box becomes **initializing** and starts onboarding on its own. If it is still `awaiting-ip` after five minutes the address never came, a terminated instance, or a subnet with no route out, and it becomes **error** with that in the note |
| A freshly launched box shows **initializing** | correct, not a fault. The account has given it an address, and an instance is not reachable until sshd is listening, typically 30–60 s. `initializing` is deliberately not `offline`, so nothing calls the box dead while it boots. Still initializing after 5 minutes means it never answered ssh at all: it becomes **error** and the note says so (terminated, or in a subnet this machine cannot dial). `i` on the box shows `state age`, how long it has been that way |
| A box was launched long ago and still has no address in the pane | check `aws ls`, if the account has no public address for it either, the subnet does not map one and this machine can never dial the box. Every launch asks for a public address explicitly, so this means the subnet has no route to an internet gateway |
